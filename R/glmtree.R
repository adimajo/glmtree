...
#' glmtree(x = data[,c("x1", "x2")], y = data$y)

glmtree <- function(x, y, K = 10, iterations = 200, test=F, validation=F, proportions = c(0.3), criterion = "bic", ctree_controls = partykit::ctree_control(alpha = 0.1, minbucket = 100, maxdepth = 5)) {
  ...
  for (i in 1:iterations) {
    ...
    for (c_iter in 1:nlevels(df$c_hat)) {
      ...
      data_train = data_train[sapply(data_train, function(x) !is.factor(x) | nlevels(factor(x)) > 1)]

      reglogs_c_hat[[c_iter]] <- stats::glm(y ~ ., data = data_train, family=stats::binomial(link="logit"))
      predictions_log[,c_iter] <- predict_tweaked(model = reglogs_c_hat[[c_iter]], df = df, c_iter = c_iter)
    }
    ...
    for (c_iter in 1:nlevels(df$c_map)) {
      ...
      data_train_c_map = df[df$c_map==levels(df$c_map)[c_iter],c("y",colnames(x))]
      data_train_c_map = data_train_c_map[sapply(data_train_c_map, function(x) !is.factor(x) | nlevels(factor(x)) > 1)]

      reglogs_c_map[[c_iter]] <- stats::glm(y ~ ., data = data_train_c_map, family=stats::binomial(link="logit"))
      if (criterion == "aic") {
        if (!validation) {
          criterion_iter[[i]] = criterion_iter[[i]] - reglogs_c_map[[c_iter]]$aic
        } else {
          criterion_iter[[i]] = criterion_iter[[i]] + sum(log(y[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter]]*stats::predict(df[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter], ], type = "response") + (1-y[ensemble[[2]]])*(1-y[ensemble[[2]]]*stats::predict(df[ensemble[[2]], ], type = "response"))))
        }
      } else if (criterion == "bic") {
        if (!validation) {
          criterion_iter[[i]] = criterion_iter[[i]] + 2*stats::logLik(reglogs_c_map[[c_iter]])-log(length(ensemble[[1]] %in% df$c_map==levels(df$c_map)[c_iter]))*length(reglogs_c_map[[c_iter]]$coefficients)
        } else {
          criterion_iter[[i]] = criterion_iter[[i]] + sum(log(y[ensemble[[2]]]*stats::predict(reglogs_c_map[[c_iter]], newdata = df[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter], ], type = "response") + (1-y[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter]])*(1-y[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter]]*stats::predict(reglogs_c_map[[c_iter]], newdata = df[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter], ], type = "response"))))
        }
      } else if (!validation) {
        criterion_iter[[i]] = rbind(criterion_iter[[i]],cbind(reglogs_c_map[[c_iter]]$y, reglogs_c_map[[c_iter]]$fitted.values))
      } else {
        criterion_iter[[i]] = rbind(criterion_iter[[i]],cbind(y[ensemble[[2]] %in% 1:n], stats::predict(reglogs_c_map[[c_iter]], newdata = df[ensemble[[2]] %in% 1:n & df$c_map==levels(df$c_map)[c_iter],], type = "response")))
      }
    }

    message(paste("The", criterion, "criterion for iteration", i, "is", criterion_iter[[i]]))

    # Mise à jour éventuelle de la meilleure solution actuelle (+burn-in de 100 itérations)
    if (i>=50 & criterion_iter[[i]] > criterion_iter[[current_best]]) {
    ...
    if (nlevels(factor(df$c_hat))>1) {
      link = partykit::ctree(c_hat ~ ., data = df[,c("c_hat",colnames(x))] %>% dplyr::mutate_at("c_hat", as.factor), control = ctree_controls)
    } else break
    ...
    df$c_map <- factor(apply(stats::predict(link,df,type="prob"),1,function(p) names(which.max(p))))

    # Tirage du nouveau c_hat
    # p <- prop.table(stats::predict(link,df)*(df$y*predictions_log + (1-df$y)*(1-predictions_log)),1)
    p <- prop.table(stats::predict(link,df,type="prob")*(df$y*predictions_log + (1-df$y)*(1-predictions_log)),1)
    df$c_hat <- factor(apply(p,1,function(row) sample(levels(df$c_hat),1,prob = row)))
  }

  return(methods::new(Class = "glmtree", parameters = list(K = K, iterations = iterations, test=test, validation=validation, proportions = proportions, criterion = criterion, ctree_controls = ctree_controls), best.tree = list(tree = best_link, glms = best_reglogs), performance = list(performance = criterion_iter[[current_best]], criterionEvolution = criterion_iter)))
}
