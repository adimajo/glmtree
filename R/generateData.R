...
#' @param visualize Whether (T) or not (F) to plot the generated data.
#' @keywords glmtree
...
generateData <- function(n = 100, scenario = "tree", visualize = F) {

  if (n < 10) {stop(simpleError("Needs more observations. Use at least n = 50."))}

  if (!is.logical(visualize)) {stop(simpleError("Parameter visualize needs to be logical."))}
  ...
  if (scenario == "tree") {
    ...
    for (c in seq(2,2*n_clusters,2)) {
      set.seed(c)
      x_cont = matrix(stats::rnorm(floor(n/n_clusters)*2,mean = 0, sd = 1.5), nrow = floor(n/n_clusters), ncol = 2)
      x_cat = replicate(floor(n/n_clusters),sample(c(c-1,c),1,c(0.5,0.5),replace=TRUE))
      x = cbind(x_cat,x_cont)
      log_odd = apply(x_cont, 1, function(row) theta[[c/2]][1] + t(theta[[c/2]][2:3]) %*% row)
      y = stats::rbinom(floor(n/n_clusters),1,1/(1+exp(-log_odd)))
      data[(1+(c/2-1)*floor(n/n_clusters)):(c/2*floor(n/n_clusters)),] <- cbind(x,y,rep(c/2,floor(n/n_clusters)))
      vraisemblance_generation[c] <- sum(log_odd)
    }

    if (n - 3*floor(n/n_clusters) > 0) {
      x_cont = matrix(stats::rnorm((n - 3*floor(n/n_clusters))*2,mean = 0, sd = 1.5), nrow = n - 3*floor(n/n_clusters), ncol = 2)
      x_cat = replicate(n - 3*floor(n/n_clusters),sample(c(c-1,c),1,c(0.5,0.5),replace=TRUE))
      x = cbind(x_cat,x_cont)
      log_odd = apply(x_cont, 1, function(row) theta[[c/2]][1] + t(theta[[c/2]][2:3]) %*% row)
      y = stats::rbinom(n - 3*floor(n/n_clusters),1,1/(1+exp(-log_odd)))
      data[(3*floor(n/n_clusters)+1):n,] <- cbind(x,y,rep(c/2,n - 3*floor(n/n_clusters)))
      vraisemblance_generation[c] <- vraisemblance_generation[c] + sum(log_odd)
    }
    ...
    for (c in 1:n_clusters) {
      set.seed(c)
      x = matrix(stats::rnorm(floor(n / n_clusters)*2, mean = 3*c, sd = 1), nrow = floor(n / n_clusters), ncol = 2)
      log_odd = apply(x, 1, function(row) theta[1] + t(theta[2:3]) %*% row)
      y = stats::rbinom(floor(n / n_clusters),1,1/(1+exp(-log_odd)))
      data[(1+(c-1)*floor(n / n_clusters)):(c*floor(n / n_clusters)),] <- cbind(x,y)
      vraisemblance_generation[c] <- sum(log_odd)
    }

    if (n - 3*floor(n/n_clusters) > 0) {
      x = matrix(stats::rnorm((n - 3*floor(n/n_clusters))*2, mean = 3*c, sd = 1), nrow = (n - 3*floor(n/n_clusters)), ncol = 2)
      log_odd = apply(x, 1, function(row) theta[1] + t(theta[2:3]) %*% row)
      y = stats::rbinom(n - 3*floor(n/n_clusters),1,1/(1+exp(-log_odd)))
    ...
    if (visualize) {
      graphics::plot(data$x1,data$x2, col = data$y+1,xlab="First coordinate",ylab="Second coordinate")
    }

    return(data)

  } else stop(simpleError("Invalid scenario."))
